...
Matrix PathSimulator::path() const
{
	...
	for (size_t time_idx = 1; time_idx < _time_points.size(); time_idx++)
	{
		...
		double vol = path[time_idx - 1][1];
		current_values = next_step(time_idx - 1, asset_price, vol);
...
Vector EulerPathSimulator::next_step(const size_t & time_idx, const double & asset_price, const double &vol) const
{
	Vector next_values;
	// DT = t_{k+1} - t_k
	...
	// + DT * k * (theta - S_{t_k})
	next_vol = delta_time * _model->drift_term_vol(asset_price);

	// sqrt(DT) * (W_{t_{k+1}} - W_{t_k})
	std::mt19937 generator = std::mt19937(std::chrono::system_clock::now().time_since_epoch().count());
	std::normal_distribution<double> distribution(0., 1.);
	double delta_brownian_vol = std::sqrt(delta_time) * distribution(generator);

	next_vol = _model->diffusion_term_vol(asset_price, vol) * delta_brownian_vol;
	...
	// + V(t_k, S_{t_k}) * sqrt(DT) * N(0,1)
	next_asset_price += _model->diffusion_term_spot(asset_price, next_vol) * _model->correlation() * delta_brownian_vol + std::sqrt(1 - pow(_model->correlation(),2)) * delta_brownian_spot;
	next_values.push_back(next_asset_price);
	next_values.push_back(next_vol);
	return next_values;
}
